function [data]=read_S2_MSIL2A_safe_PRD_struct(safe_data)

addpath(genpath('.\tools'));
granule=dir([safe_data,'\GRANULE\S2A*']);
...
IMG_path=[granule(1).folder,'\',granule(1).name,'\IMG_DATA\'];
datalist=dir([IMG_path,'*SCL*.jp2']);
data.SCL=double(imread([IMG_path,datalist(1).name]));
